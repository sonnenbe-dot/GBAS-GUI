...
def main():
    ...
    calculate_likelihoods(mergedout_path, output_qualityscores_path, allelesout_path, samplelist_path, outputpath_multiple, 0.7, 1)
    ...
    for header, data in parse_fastq(mergedoutfilepath).items():
        ...
        for i, char_el in enumerate(quality):
            if (i in sites.keys()):
                ...
                out[header][i][coverted_quality] = (mismatch_prob, match_prob, calc_helper(mismatch_prob), calc_helper(match_prob))
...
def calculate_likelihoods(mergedout_path : Path, output_qualityscores_path : Path, allelesout_path : Path, samplelist_path : Path, outputpath_likelihoods : Path, consensusthreshold : float, indexcomboposition : int):
    # quality_scores = {}
    ...
    for fastafile in allelesout_path.iterdir():
        ...
        print("File " + fastafile.name + " has " + str(len(possible_genotypes)) + " possible genotypes and " + str(len(genotype_pairs_diploid)) + " possible diploid genotype pairs. \n\n")
        ...
        for genotype_pair_diploid in genotype_pairs_diploid:
            ...
            for header, sequence in fasta_dict.items():
                ...
                qualities = [rest["BaseErrorProbability"] for site, rest in quality_scores_dict[header_merged].items()]
