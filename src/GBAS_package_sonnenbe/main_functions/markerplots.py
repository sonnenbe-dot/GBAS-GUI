...
def makematrix_cleaned(lengthmatrixjsonpath : Path, markerplots_path_excel : Path, ploidy : str, primersdict : dict, samples_dict : dict):
    ...
    with open(markerplots_path_excel, 'w', newline='') as outcsv:
        ...
        first_locus = list(markermatrix_dict.keys())[0]
        for sample in sample_list:
            row = []
            if (sample in markermatrix_dict[first_locus]["Samples"]):
                row.append(sample)
                for locus in markermatrix_dict.keys():
...
def runMarkerplots_haploid(markerstatistics_path_json : Path, markerplots_path : Path, markerplots_name : str, primersdict : dict, min_count : int, consensusthreshold : float, lengthwindow : str):
    statistics_dict = {}
    print("AM HAPLOID")
    ...
    with PdfPages(markerplots_path_pdf) as pdf:
        ...
        for primer, rest in statistics_dict.items():
            ...
            for sample, rest2 in rest["Samples"].items():
                ...
                if ((total_count > min_count) and (number_counts > 1)):
                    if ((max_count/total_count) >= consensusthreshold):
                        ...
                        print("Reached1")
                        ax.text(0.5, 1.02, "Homozygous, read length and count is: " + str(max_length) + ": " + str(max_count) + " Total Count: " + str(total_count), transform=ax.transAxes, ha="center", va="bottom", fontsize=9, color="black")
                        ax.bar(max_length, max_count_relative, width=0.1, color="black", edgecolor="black")
                        ...
                        ax.text(0.5, 1.02, "Needs manual check, read length and count is: " + str(max_length) + ": " + str(max_count) + "and " + str(max_length2) + ": " + str(max_count2) + ", Total Count: " + str(total_count), transform=ax.transAxes, ha="center", va="bottom", fontsize=9, color="grey")
                        ax.bar(max_length, max_count_relative, width=0.1, color="black", edgecolor="black")
                        #ax.bar(max_length2, max_count2_relative, width=0.1, color="red", edgecolor="red")
                        ax.text(x=max_length, y=max_count_relative + 0.5, s=str(max_length), ha="center", va="bottom", fontsize=8, color="red", weight="bold")
                        ...
                        if ("LengthBoundaries" in rest):
                            ...
                            for boundary in rest["LengthBoundaries"]:
                                text_to_add += "[" + str(boundary[0]) + "," + str(boundary[1]) + "], "
                            ax.text(0.5, -0.3, text_to_add, transform=ax.transAxes, ha="center", va="top", fontsize=9, color="black")
                elif ((total_count > min_count) and (number_counts == 1)):
                    length_matrix_dict[primer]["Samples"][sample]["LengthAlleles"][max_length] = max_count
                    ...
                    ax.text(0.5, 1.02, "Too little read count", transform=ax.transAxes, ha="center", va="bottom", fontsize=9, color="grey")
    ...
    with PdfPages(markerplots_path_pdf) as pdf:
        ...
        for primer, rest in statistics_dict.items():
            ...
            for sample, rest2 in rest["Samples"].items():
                ...
                potstutt = max_length - repsiz
                potstutt2 = max_length - 2 * repsiz
                ...
                if ((total_count > min_count) and (number_counts > 1)): #max_count is count of highest length peak, total_count is total_count (sum of all counts)
                    ...
                    if ((b1 >= b2) & (b1 >= b3)):
                        ...
                        # fifth case - only one repeat size difference but second largest peak is minimum size of 0.75 % of the largest one, 
                            ...
                            length_matrix_dict[primer]["Samples"][sample]["LengthAlleles"][max_length2] = max_count2
                            length_matrix_dict[primer]["Samples"][sample]["Information"] = "Needs manual check"
                            ax.text(0.5, 1.02, "Needs manual check, read length and count is: " + str(max_length) + ": " + str(max_count) + "and " + str(max_length2) + ": " + str(max_count2) + ", Total Count: " + str(total_count), transform=ax.transAxes, ha="center", va="bottom", fontsize=9, color="red")
                ...
                elif ((total_count > min_count) and (number_counts == 1)): #total_count instead of max_count
                    #print("Reached10")
                    length_matrix_dict[primer]["Samples"][sample]["LengthAlleles"][max_length] = max_count
                    length_matrix_dict[primer]["Samples"][sample]["Information"] = "Homozygous, only one read length present"
                    ax.text(0.5, 1.02, "Homozygous, only one read length present: " + str(max_length) + ": " + str(max_count) + " Total Count: " + str(total_count), transform=ax.transAxes, ha="center", va="bottom", fontsize=9, color="black")
                    ...
                    ax.text(0.5, 1.02, "Too little read count", transform=ax.transAxes, ha="center", va="bottom", fontsize=9, color="red")
                    #Testing it for testfiles!!!
                    # ax.bar(max_length, max_count_relative, width=0.1, color="black", edgecolor="blue")
                    # ax.text(x=max_length, y=max_count_relative + 0.5, s=str(max_length), ha="center", va="bottom", fontsize=8, color="black", weight="bold")
                    length_matrix_dict[primer]["Samples"][sample]["Information"] = "Too Little Read Count"
                    if ("LengthBoundaries" in rest):
                        text_to_add = "Length Boundaries: "
