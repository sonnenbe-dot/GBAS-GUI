...
def runMarkerplots_GUI(textbox_pipeline : ctk.CTkTextbox, paramsdict : dict, primers_dict : dict, samples_dict : dict):
    ...
    runMarkerplots(markerstatistics_path_json, markerplots_path, primers_dict, min_count, consensusthreshold, lengthwindow)
...
def runMarkerplots(markerstatistics_path_json : Path, markerplots_path : Path, primersdict : dict, min_count : int, consensusthreshold : float, lengthwindow : str):
    statistics_dict = {}
        ...
        for primer, rest in statistics_dict.items():
            ...
            for sample, rest2 in rest["Samples"].items():
                ...
                if (total_count != 0):
                    ...
                    # min_length = min(lengths_sorted)
                    # max_length = max(lengths_sorted)

                    # lengths = [int(k) for k in rest2["Lengths-Counts"].keys()]
                    # counts = [int(k) for k in rest2["Lengths-Counts"].values()]
                    # count_freqs = [(int(k)/int(total_count))*100 for k in rest2["Lengths-Counts"].values()]
                    # min_length = min(lengths)
                    # max_length = max(lengths)

                    # #Find first and second peak count with corresponding lengths
                    # list_counts = list(rest2["Lengths-Counts"].values())
                    # list_counts_ordered = sorted(list_counts, reverse = True)
                    # number_counts = len(list_counts_ordered)
                    ...
                    # max_length = [int(length) for length, count in rest2["Lengths-Counts"].items() if (count == max_count)][0]
                    # min_length = [int(length) for length, count in rest2["Lengths-Counts"].items() if (count == min_count)][0]
                ...
                #If for max count we have n lengths, consider the first n lengths my alleles as possible length alleles
                ...
                # max_count = max(rest2["Lengths-Counts"].values())
                # min_count = min(rest2["Lengths-Counts"].values())
                # reads_with_max_count = [length for length, count in rest2["Lengths-Counts"].items() if (count == max_count)]
                # reads_with_min_count = [length for length, count in rest2["Lengths-Counts"].items() if (count == min_count)]
